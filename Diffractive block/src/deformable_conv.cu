...
{
  const int kT = kernel.size(2);  // kernel 尺寸
  const int kH = kernel.size(3);
  const int kW = kernel.size(4);
  const int oC = output.size(1);  // output 尺寸
  const int oT = output.size(2);
  const int oH = output.size(3);
  const int oW = output.size(4);
  const int iC = input.size(1);  // input 尺寸
  const int iT = input.size(2);
  const int iH = input.size(3);
  const int iW = input.size(4);
  const int channel_multiplier = oC / iC;  // 倍增器 通常一致即==1
  const int dilationT = dilationT_;
  const int dilationH = dilationH_;
  const int dilationW = dilationW_;
  const int num_output = output.size(0) * output.stride(0);

  CUDA_KERNEL_LOOP(index, num_output) {
    const int out_col = index % oW;  // 输出指针的位置 行 列 高
    ...
    const int in_col_start = out_col * strideW - paddingW;  // 输入指针的位置 行 列 高
    const int in_row_start = out_row * strideH - paddingH;
    const int in_frame_start = out_frame * strideT - paddingT;

    accscalar_t sum = 0;
    const scalar_t *kernel_ptr = kernel[out_channel].data();  // 读取kernel指针
      ...
      for (int k_row = 0; k_row < kH; ++k_row) {
        ...
        for (int k_col = 0; k_col < kW; ++k_col) {
          ...
          if (in_frame >= 0 && in_row >= 0 && in_col >= 0 &&
            ...
            // 就改这行！！
            sum += 1.0 / ( 1.0 + exp(-((*(input_ptr) - op1) * 4 - 4)));  // 累加卷积核计算
          }
          input_ptr += dilationW;  // 扩张没啥用
        }
...
conv_depthwise3d_cuda_backward_input_kernel(
    ...
    const PackedTensorAccessor32<scalar_t, 5> kernel,
    const PackedTensorAccessor32<scalar_t, 5> output,
  ...
  CUDA_KERNEL_LOOP(index, num_input) {
    const int in_col = index % iW;  // 反过来了 输入变输出 输出变输入
    ...
    for (int k_chn = in_channel * channel_multiplier;
      ...
      const scalar_t* out_ptr = output[batch][k_chn].data();
...
Tensor conv_depthwise3d_cuda(
  ...
  int64_t kernel_size_int[3] = {kernel_size_one,kernel_size_one,kernel_size_one};
  int64_t stride_int[3] = {stride_one,stride_one,stride_one};
  int64_t padding_int[3] = {padding_one,padding_one,padding_one};
  int64_t dilation_int[3] = {dilation_one,dilation_one,dilation_one};
  IntArrayRef kernel_size = IntArrayRef(kernel_size_int, 3);
  IntArrayRef stride = IntArrayRef(stride_int, 3);
  IntArrayRef padding = IntArrayRef(padding_int, 3);
  IntArrayRef dilation = IntArrayRef(dilation_int, 3);
...
Tensor _depthwise_3d_backward_cuda_out(
    Tensor& grad_input,
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& weight,
    ...
    const std::array<bool, 3> output_mask)
{
  ...
  const Tensor output_ = output.contiguous();
